	...
	i.factory("$$uisDebounce", ["$timeout", function(e) {
		...
		function f(e, t) {
			...
			for(s = 0; s < t.length; s++)
				for(i = 0; i < e.length; i++) e[i].name == [t[s]] && c.push(e[i]);
			return c
		}
		...
		function b(e) {
			...
			switch(e) {
				case t.DOWN:
					if(!y.open && y.multiple) y.activate(!1, !0);
					else if(y.activeIndex < y.items.length - 1)
						for(var i = ++y.activeIndex; m(y.items[i]) && i < y.items.length;) y.activeIndex = ++i;
					break;
				case t.UP:
					var c = 0 === y.search.length && y.tagging.isActivated ? -1 : 0;
					if(!y.open && y.multiple) y.activate(!1, !0);
					else if(y.activeIndex > c)
						for(var n = --y.activeIndex; m(y.items[n]) && n > c;) y.activeIndex = --n;
					break;
				case t.TAB:
		...
		y.isEmpty = function() {
			...
			if(y.disabled || y.open) y.open && !y.searchEnabled && y.close();
			else {
			...
			y.parserResult.keyName && (o(), y.parserResult.source = u("$uisSource" + y.parserResult.filters), s.$watch(l, function(e, t) {
				...
				var t = y.selected;
				if(y.isEmpty() || angular.isArray(t) && !t.length || !y.multiple || !y.removeSelected) y.setItemsFn(e);
		...
		y.sizeSearchInput = function() {
			...
			var s;
			if(s = window.clipboardData && window.clipboardData.getData ? window.clipboardData.getData("Text") : (e.originalEvent || e).clipboardData.getData("text/plain"), s = y.search + s, s && s.length > 0)
				if(y.taggingTokens.isActivated) {
			...
			link: function(c, n, l, a) {
				...
				function d(e, t) {
					...
					if(angular.isArray(e))
						for(var i = angular.copy(e), c = 0; c < i.length; c++)
							if(void 0 === p.tagging.fct) i[c] + " " + p.taggingLabel === t && (s = c);
							else {
								var n = i[c];
				...
				p.multiple = !0, p.focusInput = p.searchInput, h.$isEmpty = function(e) {
					...
					var t, s = p.parserResult && p.parserResult.source(c, {
							$select: {
								search: ""
							}
						}),
					...
					if(t.isVerticalMovement(e.which) || c.$evalAsync(function() {
						...
						var s, i, n, l, a = angular.copy(p.items),
		...
		return {
			...
			link: function(c, n, l, a) {
				...
				o.$parsers.unshift(function(t) {
					...
					if(e(t)) return t;
					var s, i = r.parserResult && r.parserResult.source(c, {
							$select: {
								search: ""
							}
						}),
